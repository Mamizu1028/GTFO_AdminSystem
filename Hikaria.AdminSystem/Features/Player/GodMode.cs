...
public class GodMode : Feature
{
    ...
    private static void ToggleIgnoreAllDamage([PlayerAgentSuggestorTag] PlayerAgent playerAgent)
    {
        if (!GodModeLookup.TryGetValue(playerAgent.Owner.Lookup, out var entry))
        {
            ConsoleLogs.LogToConsole("输入有误", LogLevel.Error);
            return;
        }
        entry.IgnoreAllDamage = !entry.IgnoreAllDamage;
        ConsoleLogs.LogToConsole($"已{(entry.IgnoreAllDamage ? "启用" : "禁用")} {playerAgent.GetColoredNameWithoutRichTextTags()} 免疫伤害");
    }

    [Command("IgnoreInfection")]
    private static void ToggleIgnoreInfection([PlayerAgentSuggestorTag] PlayerAgent playerAgent)
    {
        if (!GodModeLookup.TryGetValue(playerAgent.Owner.Lookup, out var entry))
        {
            ConsoleLogs.LogToConsole("输入有误", LogLevel.Error);
        ...
        ConsoleLogs.LogToConsole($"已{(entry.IgnoreInfection ? "启用" : "禁用")} {playerAgent.GetColoredNameWithoutRichTextTags()} 免疫感染");
    }

    [Command("CannotDie")]
    private static void ToggleCannotDie([PlayerAgentSuggestorTag] PlayerAgent playerAgent)
    {
        if (!GodModeLookup.TryGetValue(playerAgent.Owner.Lookup, out var entry))
        {
            ConsoleLogs.LogToConsole("输入有误", LogLevel.Error);
        ...
        ConsoleLogs.LogToConsole($"已{(entry.CannotDie ? "启用" : "禁用")} {playerAgent.GetColoredNameWithoutRichTextTags()} 免疫倒地");
    }
    ...
    private class PlayerLocomotion__ChangeState__Patch
    {
        private static void Postfix(PlayerLocomotion __instance, PlayerLocomotion.PLOC_State state)
        {
            ...
            ConsoleLogs.LogToConsole($"<color=green>已复活玩家 {player.GetColoredNameWithoutRichTextTags()}</color>");
        }
    }
