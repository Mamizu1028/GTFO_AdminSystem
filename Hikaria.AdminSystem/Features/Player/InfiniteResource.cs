...
public class InfiniteResource : Feature
{
    ...
    private static void ToggleInfResource([PlayerAgentSuggestorTag] PlayerAgent playerAgent)
    {
        if (!InfResourceLookup.TryGetValue(playerAgent.Owner.Lookup, out var entry))
        {
            ConsoleLogs.LogToConsole("输入有误", LogLevel.Error);
            return;
        }
        entry.InfResource = !entry.InfResource;
        ConsoleLogs.LogToConsole($"已{(entry.InfResource ? "启用" : "禁用")} {playerAgent.GetColoredNameWithoutRichTextTags()} 无限资源");
    }

    [Command("InfSentry", "无限哨戒炮")]
    private static void ToggleInfSentry([PlayerAgentSuggestorTag] PlayerAgent playerAgent)
    {
        if (!InfResourceLookup.TryGetValue(playerAgent.Owner.Lookup, out var entry))
        {
            ConsoleLogs.LogToConsole("输入有误", LogLevel.Error);
        ...
        ConsoleLogs.LogToConsole($"已{(entry.InfSentry ? "启用" : "禁用")} {playerAgent.GetColoredNameWithoutRichTextTags()} 无限哨戒炮");
    }
    ...
    {
        if (state == (int)eGameStateName.InLevel || state == (int)eGameStateName.Lobby)
        {
